...
describe("FeedService", () => {
  ...
  describe("getHomeFeed", () => {
    it("returns public posts for unauthenticated user without querying social graph", async () => {
      prisma.post.findMany.mockResolvedValue([{ id: "post1", createdAt: new Date() }]);
      ...
      expect(prisma.post.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            AND: expect.arrayContaining([
              expect.objectContaining({
                OR: expect.arrayContaining([expect.objectContaining({ authorId: "user1" })]),
              }),
            ]),
          }),
        }),
    ...
    it("excludes blocked users from visible author set", async () => {
      ...
      prisma.follow.findMany.mockResolvedValue([]);
      prisma.block.findMany.mockResolvedValue([{ blockerId: "user1", blockedId: "user2" }]);
      prisma.post.findMany.mockResolvedValue([]);

      await service.getHomeFeed("user1", { limit: 20 });

      const call = prisma.post.findMany.mock.calls[0][0];
      const orArray: Array<{ authorId?: string | { in?: string[] } }> = call.where.AND[0].OR;
  ...
  describe("feed enrichment", () => {
    it("includes author profile (firstName, lastName, headline) in getHomeFeed response", async () => {
      ...
      expect(result.data[0].author.profile.headline).toBe("Software Developer");
    });
  });
});
