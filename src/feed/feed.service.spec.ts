...
describe("FeedService", () => {
	...
	describe("getHomeFeed", () => {
			...
			expect(prisma.post.findMany).toHaveBeenCalledWith(
				expect.objectContaining({
					where: expect.objectContaining({
						AND: expect.arrayContaining([
							expect.objectContaining({
								authorId: expect.objectContaining({ in: expect.any(Array) }),
							}),
						]),
					}),
				}),
		...
		it("excludes blocked users from visible author set", async () => {
			...
			const call = prisma.post.findMany.mock.calls[0][0] as any;
			const authorIn: string[] = call.where.AND[0].authorId.in;
			expect(authorIn).not.toContain("user2");
			expect(authorIn).toContain("user1");
		});
