...
describe('ProfilesService', () => {
  ...
  describe('replaceLanguages', () => {
    it('should delete existing languages and create new ones', async () => {
      jest
        .spyOn(prisma.profileLanguage, 'deleteMany')
        .mockResolvedValue({ count: 1 });
      jest
        .spyOn(prisma.profileLanguage, 'createMany')
        .mockResolvedValue({ count: 1 });
      jest
        .spyOn(prisma.profileLanguage, 'findMany')
        .mockResolvedValue([{ id: 'l1' }] as any);

      const result = await service.replaceLanguages('prof-1', [
        { language: 'English', proficiency: 'NATIVE_BILINGUAL' },
      ]);

      expect(prisma.profileLanguage.deleteMany).toHaveBeenCalledWith({
        where: { profileId: 'prof-1' },
      });
      expect(prisma.profileLanguage.createMany).toHaveBeenCalledWith({
        data: [
          {
            profileId: 'prof-1',
            language: 'English',
            proficiency: 'NATIVE_BILINGUAL',
          },
        ],
        skipDuplicates: true,
      });
    ...
    it('should throw ConflictException on duplicate language P2002', async () => {
      jest
        .spyOn(prisma.profileLanguage, 'deleteMany')
        .mockResolvedValue({ count: 0 });
      jest
      ...
      await expect(
        service.replaceLanguages('prof-1', [
          { language: 'English', proficiency: 'NATIVE_BILINGUAL' as any },
        ]),
      ).rejects.toThrow(ConflictException);
    });
  });
