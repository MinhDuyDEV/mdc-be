...
describe("ProfilesController", () => {
	...
	describe("GET /profiles/:userId", () => {
		...
		describe("GET /profiles/search", () => {
			it("should call profilesService.searchProfiles with dto values", async () => {
				const dto = { q: "react developer", limit: 10, offset: 5 };
				const resultMock = { data: [{ id: "p1" }], meta: { total: 1 } };
				jest
					.spyOn(profilesService, "searchProfiles")
					.mockResolvedValue(resultMock as any);

				const result = await controller.searchProfiles(dto as any);
