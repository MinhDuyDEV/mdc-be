...
describe("EmailProcessor", () => {
  ...
  beforeEach(async () => {
    ...
    trackingService = {
      ...
      injectTrackingPixel: jest.fn().mockImplementation((html: string) => html + "<!--pixel-->"),
      rewriteLinks: jest
        .fn()
        .mockImplementation((html: string) => html.replace(/href="http/g, 'href="https://track/')),
  ...
  describe("pollPending", () => {
    ...
    it("should process pending email and inject tracking when consent exists", async () => {
      ...
      prisma.emailConsent.findMany.mockResolvedValue([{ userId: "user-1", trackingConsent: true }]);
      mailerService.sendMail.mockResolvedValue({ messageId: "abc-123" });
    ...
    it("should NOT inject tracking when no consent", async () => {
      ...
      prisma.emailDelivery.findMany.mockResolvedValue([mockDelivery]);
      prisma.user.findMany.mockResolvedValue([{ id: "user-1", email: "user@example.com" }]);
      prisma.emailConsent.findMany.mockResolvedValue([
        { userId: "user-1", trackingConsent: false },
      ]);
      mailerService.sendMail.mockResolvedValue({ messageId: "abc-123" });
