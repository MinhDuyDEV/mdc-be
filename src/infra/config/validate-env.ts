...
const VALID_PROCESS_ROLES = new Set<ProcessRole>(["api", "worker", "realtime", "all"]);
...
function parseOptionalPositiveInteger(env: RawEnv, key: string, defaultVal: number): number {
...
function parseCookieSameSite(env: RawEnv, key: string): "strict" | "lax" | "none" {
...
function parseProcessRole(value: string | undefined): ProcessRole {
  ...
  if (!VALID_PROCESS_ROLES.has(role as ProcessRole)) {
    throw new Error(`APP_PROCESS_ROLE must be one of: api, worker, realtime, all (got: ${role})`);
  }
...
export function validateEnv(env: RawEnv): AppConfig {
  ...
  return {
    ...
    healthElasticsearchTimeoutMs: parsePositiveInteger(env, "HEALTH_ELASTICSEARCH_TIMEOUT_MS"),
    ...
    healthMailerTimeoutMs: parsePositiveInteger(env, "HEALTH_MAILER_TIMEOUT_MS"),
    // OpenTelemetry
    ...
    outboxMaxRetries: parseOptionalPositiveInteger(env, "OUTBOX_MAX_RETRIES", 5),
    outboxBaseBackoffMs: parseOptionalPositiveInteger(env, "OUTBOX_BASE_BACKOFF_MS", 1000),
    outboxMaxBackoffMs: parseOptionalPositiveInteger(env, "OUTBOX_MAX_BACKOFF_MS", 60000),
    outboxLeaseTimeoutMs: parseOptionalPositiveInteger(env, "OUTBOX_LEASE_TIMEOUT_MS", 60000),
    outboxHealthLagThreshold: parseOptionalPositiveInteger(env, "OUTBOX_HEALTH_LAG_THRESHOLD", 100),
    // JWT Authentication
    jwtAccessSecret: requireString(env, "JWT_ACCESS_SECRET"),
    jwtAccessExpiresIn: parseOptionalString(env, "JWT_ACCESS_EXPIRES_IN") || "15m",
    jwtRefreshExpiresIn: parseOptionalString(env, "JWT_REFRESH_EXPIRES_IN") || "7d",
    ...
    // Rate Limiting
    throttleLoginLimit: parseOptionalPositiveInteger(env, "THROTTLE_LOGIN_LIMIT", 5),
    throttleLoginTtl: parseOptionalPositiveInteger(env, "THROTTLE_LOGIN_TTL", 60000),
    throttleRegisterLimit: parseOptionalPositiveInteger(env, "THROTTLE_REGISTER_LIMIT", 3),
    throttleRegisterTtl: parseOptionalPositiveInteger(env, "THROTTLE_REGISTER_TTL", 60000),
    ...
    throttleRefreshLimit: parseOptionalPositiveInteger(env, "THROTTLE_REFRESH_LIMIT", 10),
    throttleRefreshTtl: parseOptionalPositiveInteger(env, "THROTTLE_REFRESH_TTL", 60000),
    // Content creation rate limiting
    throttlePostCreateLimit: parseOptionalPositiveInteger(env, "THROTTLE_POST_CREATE_LIMIT", 5),
    throttlePostCreateTtl: parseOptionalPositiveInteger(env, "THROTTLE_POST_CREATE_TTL", 60000),
    ...
    throttleMessageSendLimit: parseOptionalPositiveInteger(env, "THROTTLE_MESSAGE_SEND_LIMIT", 30),
    throttleMessageSendTtl: parseOptionalPositiveInteger(env, "THROTTLE_MESSAGE_SEND_TTL", 60000),
    throttleReportCreateLimit: parseOptionalPositiveInteger(
      env,
      "THROTTLE_REPORT_CREATE_LIMIT",
      10,
    ),
    throttleReportCreateTtl: parseOptionalPositiveInteger(env, "THROTTLE_REPORT_CREATE_TTL", 60000),
    ...
    stripeApiVersion: parseOptionalString(env, "STRIPE_API_VERSION") || "2024-12-18.acacia",
    ...
    unleashUrl: parseOptionalString(env, "UNLEASH_URL") || "http://localhost:4242/api",
    ...
    emailTrackingBaseUrl:
      parseOptionalString(env, "EMAIL_TRACKING_BASE_URL") || "http://localhost:3000",
