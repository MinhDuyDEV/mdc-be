...
describe("MessagingService", () => {
  ...
  beforeEach(() => {
    ...
    recruitingPolicy = {
      canMessageCandidate: jest.fn().mockResolvedValue({ allowed: true, reason: "OPT_IN" }),
    };
    service = new MessagingService(prisma, outbox, idempotency, messagingPolicy, recruitingPolicy);
  });
  ...
  describe("sendMessage", () => {
    it("rejects non-participants with FORBIDDEN", async () => {
      ...
      await expect(service.sendMessage("user-1", "conv-1", { content: "Hello" })).rejects.toThrow(
        new ForbiddenException("NOT_A_PARTICIPANT"),
      );
    });
    ...
    it("rejects blocked users with BLOCKED_USER", async () => {
      ...
      await expect(service.sendMessage("user-1", "conv-1", { content: "Hello" })).rejects.toThrow(
        new ForbiddenException("BLOCKED_USER"),
      );
  ...
  describe("listConversations", () => {
    ...
    it("defaults missing limit before calling Prisma", async () => {
      ...
      const result = await service.listConversations("user-1", {} as CursorPaginationQueryDto);
  ...
  describe("getMessages", () => {
    it("defaults missing limit before calling Prisma", async () => {
      ...
      const result = await service.getMessages("user-1", "conv-1", {} as CursorPaginationQueryDto);

      expect(prisma.message.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 21 }));
      expect(result.meta.limit).toBe(20);
    });
  });
  ...
  describe("updateGroupConversation", () => {
    it("updates title for admin", async () => {
      ...
      const result = await service.updateGroupConversation("user-1", "conv-g1", {
        title: "New Title",
      });
  ...
  describe("addParticipant", () => {
    it("adds participant with MEMBER role", async () => {
      ...
      const result = await service.addParticipant("user-1", "conv-g1", "user-4");
    ...
    it("rejects non-admin with NOT_ADMIN", async () => {
      ...
      await expect(service.addParticipant("user-2", "conv-g1", "user-4")).rejects.toThrow(
        new ForbiddenException("NOT_ADMIN"),
      );
    });
    ...
    it("rejects non-group conversation with NOT_A_GROUP_CONVERSATION", async () => {
      ...
      await expect(service.addParticipant("user-1", "conv-d1", "user-4")).rejects.toThrow(
        new BadRequestException("NOT_A_GROUP_CONVERSATION"),
      );
    });
  });
  ...
  describe("removeParticipant", () => {
    it("self-leave sets leftAt and returns ok", async () => {
      const result = await service.removeParticipant("user-1", "conv-g1", "user-1");
    ...
    it("admin removes other participant and returns ok", async () => {
      ...
      const result = await service.removeParticipant("user-1", "conv-g1", "user-2");
    ...
    it("rejects non-admin removing other with NOT_ADMIN", async () => {
      ...
      await expect(service.removeParticipant("user-2", "conv-g1", "user-3")).rejects.toThrow(
        new ForbiddenException("NOT_ADMIN"),
      );
    });
  });
  ...
  describe("deleteMessage", () => {
    ...
    it("rejects unauthorized user with NOT_AUTHORIZED", async () => {
      ...
      await expect(service.deleteMessage("user-3", "conv-1", "msg-1")).rejects.toThrow(
        new ForbiddenException("NOT_AUTHORIZED"),
      );
    });
    ...
    it("rejects non-existent message with MESSAGE_NOT_FOUND", async () => {
      ...
      await expect(service.deleteMessage("user-1", "conv-1", "msg-999")).rejects.toThrow(
    ...
    it("rejects message in wrong conversation with MESSAGE_NOT_FOUND", async () => {
      ...
      await expect(service.deleteMessage("user-1", "conv-2", "msg-1")).rejects.toThrow(
        new NotFoundException("MESSAGE_NOT_FOUND"),
      );
    });
  });
