...
describe("RecruitingService", () => {
  ...
  describe("updateInterview", () => {
    ...
    it("updates interview fields and does not emit event for non-COMPLETED", async () => {
      ...
      const result = await service.updateInterview("user-1", "c-1", "interview-1", {
        location: "Room 42",
      });
    ...
    it("rejects when interview not found", async () => {
      ...
      await expect(service.updateInterview("user-1", "c-1", "missing", {})).rejects.toThrow(
        new NotFoundException("INTERVIEW_NOT_FOUND"),
      );
    });
  });
  ...
  describe("addInterviewer", () => {
    ...
    it("creates interviewer", async () => {
      ...
      const result = await service.addInterviewer("user-1", "c-1", "interview-1", "target-1");
    ...
    it("rejects when interview not found", async () => {
      ...
      await expect(service.addInterviewer("user-1", "c-1", "missing", "target-1")).rejects.toThrow(
        new NotFoundException("INTERVIEW_NOT_FOUND"),
      );
    });
  });
      ...
      expect(prisma.interview.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            AND: expect.arrayContaining([{ companyId: "c-1" }, { applicationId: "app-1" }]),
          }),
        }),
      );
    });
  ...
  describe("listScorecards", () => {
    ...
    it("returns paginated results", async () => {
      const rows = [{ id: "sc-1", createdAt: new Date("2026-06-01"), sections: [] }];
  ...
  describe("sendOffer", () => {
    ...
    it("rejects when offer not found", async () => {
      ...
      await expect(service.sendOffer("user-1", "c-1", "missing")).rejects.toThrow(
        new NotFoundException("OFFER_NOT_FOUND"),
      );
    });
    ...
    it("rejects when offer not in DRAFT", async () => {
      ...
      await expect(service.sendOffer("user-1", "c-1", "offer-1")).rejects.toThrow(
        new ConflictException("OFFER_NOT_DRAFT"),
      );
    });
  });
  ...
  describe("respondToOffer", () => {
    ...
    it("rejects when offer not found or not SENT", async () => {
      ...
      await expect(service.respondToOffer("user-1", "missing", true)).rejects.toThrow(
        new NotFoundException("OFFER_NOT_FOUND_OR_NOT_SENT"),
      );
    });
    ...
    it("rejects when user is not the candidate", async () => {
      ...
      await expect(service.respondToOffer("other-user", "offer-1", true)).rejects.toThrow(
        new ForbiddenException("NOT_CANDIDATE_FOR_OFFER"),
      );
    });
  });
});
