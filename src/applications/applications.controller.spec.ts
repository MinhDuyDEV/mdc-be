...
describe("ApplicationsController", () => {
	...
	it("submit delegates to service.submitApplication", async () => {
		...
		const result = await controller.submit(candidate, "job-1", dto as never);
	...
	it("listMine forwards user.id and pagination query", async () => {
		...
		await controller.listMine(candidate, { limit: 20 } as never);
	...
	it("listForJob delegates to listEmployerApplications", async () => {
		...
		await controller.listForJob(candidate, "job-1", { limit: 10 } as never);
