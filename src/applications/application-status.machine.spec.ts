...
describe("evaluateTransition — candidate WITHDRAW paths", () => {
	it.each([
		...
		[S.INTERVIEWING],
		[S.OFFER],
	])("%s → WITHDRAWN with candidate returns ok", (from) => {
...
describe("evaluateTransition — terminal source", () => {
	...
	it.each(
		terminals.flatMap((from) => targets.map((to) => [from, to])),
	)("from terminal %s → %s returns APPLICATION_TERMINAL", (from, to) => {
...
describe("evaluateTransition — actor rule violations", () => {
	...
	it.each([
		...
		[S.INTERVIEWING],
		[S.OFFER],
	])("%s → WITHDRAWN with recruiter returns INSUFFICIENT_ACTOR_ROLE", (from) => {
...
describe("evaluateTransition — system actor", () => {
	...
	it.each([
		...
		[S.INTERVIEWING],
		[S.OFFER],
	])("system cannot WITHDRAW from %s", (from) => {
...
describe("evaluateTransition — SUBMITTED as target is always invalid", () => {
	it.each([
		...
		["candidate" as const],
		["system" as const],
	])("REVIEWED → SUBMITTED with %s returns INVALID_STATUS_TRANSITION", (actor) => {
...
describe("ALLOWED_TRANSITIONS — graph shape", () => {
	...
	it.each([
		...
		S.REJECTED,
		S.WITHDRAWN,
	])("terminal %s has 0 allowed targets", (status) => {
